...
- Run statement-service and config-server in separate containers or as part of a Kubernetes deployment
- Ensure Vault is configured with secure storage backends and network controls
- Harden Keycloak (TLS, strong credentials, correct realm and client configuration)
- Configure proper DB migrations (e.g. Flyway/Liquibase) for `statement-service` schemas
- Tune connection pools and JVM memory for expected traffic
- Add monitoring alerts based on health, error rates, and latency (via Prometheus metrics)
